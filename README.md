...
├── test-state-change-hook.py  # 状态变更钩子测试（P2-11，95 条用例，含 main() 集成 15 条 + KA-100 退出码契约回归 + KA-101 baseline 口径对齐 + KA-416 CLI 只读重试回归 7 条 + KA-424 退出码分轨 read-error→3 / write-error→1 共 23 条）
...
dashboard/                    # 智能看板生产交付物（KA-103 入库，原型 B 落地）
